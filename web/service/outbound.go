...
var testSemaphore = make(chan struct{}, 5) // Max 5 concurrent tests
...
func (s *OutboundService) TestOutbound(outboundJSON string, testURL string, allOutboundsJSON string) (*TestOutboundResult, error) {
	...
	// Semaphore එකෙන් slot එකක් ගන්න (Blocking)
    select {
    case testSemaphore <- struct{}{}:
        defer func() { <-testSemaphore }() // Slot එක නිදහස් කරන්න
    case <-time.After(30 * time.Second):
        return &TestOutboundResult{
            Success: false,
            Error:   "Timeout waiting for test slot. Too many concurrent tests.",
        }, nil
    }
