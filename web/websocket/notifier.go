...
// BroadcastRouting broadcasts routing config update to all connected clients
func BroadcastRouting(routingConfig any) {
    hub := GetHub()
    if hub != nil {
        hub.Broadcast(MessageTypeRouting, routingConfig)
    }
}